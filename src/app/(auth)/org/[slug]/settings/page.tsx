...
export default function OrganizationSettingsBySlugPage() {
	...
	const targetOrg = organizations.find((org) => org.slug === params.slug);
	...
	useEffect(() => {
		if (!sessionPending && !orgLoading && session && !targetOrg) {
			router.push("/dashboard");
		}
	}, [session, sessionPending, orgLoading, targetOrg, router]);

	useEffect(() => {
		if (targetOrg && activeOrganization?.id !== targetOrg.id) {
			organization.setActive({ organizationId: targetOrg.id });
		}
	}, [targetOrg, activeOrganization?.id]);
	...
	if (!session || !targetOrg) {
		return null;
	}

	const displayOrg = targetOrg;
	...
	return (
		<div className="container mx-auto max-w-4xl space-y-6">
			<div className="flex items-center justify-between">
				<div>
					<h1 className="text-2xl font-bold">{displayOrg.name}</h1>
